...
int G = py::len(resp); //resp is list
int openmp = 0;
...
Phi1 = phi[0].cast<arma::mat>();
Phi2 = phi[1].cast<arma::mat>();
Phi3 = phi[2].cast<arma::mat>();
...
lossBeta = softmaxloss(eevBeta, zeta(z), lse);
...
if(alg == "npg"){/////////////////NPG algorithm from chen2016
...
GradlossX = gradloss(PhitZ, PhitPhiX, eevX, ng, zeta(z), lse);
lossX = softmaxloss(eevX, zeta(z), lse);
...
lossProp = softmaxloss(eevProp, zeta(z), lse);
...
lossProp = softmaxloss(-eev(PhiProp, Z, ng), zeta(z), lse);
...
return py::make_tuple(carma::cube_to_arr(Coef),
                      ...
                      carma::mat_to_arr(Stops),
                      openmp 
                      /* carma::cube_to_arr(Z),
                      carma::mat_to_arr(Phi1),carma::mat_to_arr(Phi2),carma::mat_to_arr(Phi3),
                      n1,n2,n3 */
                      ); 
...
RESP(i, 0) = resp[i].cast<arma::mat>(); //ngx1 matrices
PHI(i, 0) = phi[i].cast<arma::mat>(); //ngxp matrices
n(i) = PHI(i, 0).n_rows;

}

int p = PHI(0, 0).n_cols;//Rcpp::as<arma::mat>(Phi[0]).n_cols;
...
-eev_f(field_mult(PHI, arma::zeros<mat>(p, 1)), RESP, n), n, 42, lse));
...
absgradzeroall = abs(gradloss_f(PHItRESP, PHItPHIX, eevX, n, zeta(z), lse)); //todo gradloss
...
GradlossX = gradloss_f(PHItRESP, PHItPHIX, eevX, n, zeta(z), lse);
lossX = softmaxloss(eevX, zeta(z), lse);
...
py::tuple  WT(arma::mat x, int dim, string wf, int J, int p1, int p2, int p3){
...
delete [] g;
delete [] h;
return py::make_tuple(carma::mat_to_arr(out)); 
}

py::tuple IWT(arma::mat x, int dim, string wf, int J, int p1, int p2, int p3){
arma::mat out(p1, p2 * p3);
int L = get_L(wf); 
double *g = nullptr, *h = nullptr;
g = new double[L]; 
...
delete [] g;
delete [] h;   
return py::make_tuple(carma::mat_to_arr(out, true)); 
}